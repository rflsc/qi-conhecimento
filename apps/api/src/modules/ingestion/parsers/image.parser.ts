...
export class ImageParser implements DocumentParser {
  ...
  private async parseWithVision(buffer: Buffer): Promise<ParseResult> {
    ...
    const apiKey = runtime.provider === 'openai' ? runtime.apiKey : '';
    if (!apiKey) {
      throw new Error(
        'OCR requer PARSER_SERVICE_URL (Docling) ou chave OpenAI em Admin → Configurações (Vision API)',
      );
    }
    ...
    const response = await openai.chat.completions.create({
      model: runtime.provider === 'openai' ? runtime.model : 'gpt-4o-mini',
      messages: [
        {
          role: 'user',
          content: [
