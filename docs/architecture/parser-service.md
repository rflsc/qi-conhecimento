...
Microserviço Python que converte documentos técnicos (PDF, imagens) em **Markdown estruturado**, preservando tabelas e hierarquia de seções. A API NestJS chama via `DoclingClient` quando `PARSER_SERVICE_URL` está definido; faz **fallback automático** para `pdf-parse` / Vision API se indisponível.
...
| `POST` | `/v1/parse` | `multipart/form-data` (campo `file`) → Markdown |
...
| `PARSER_SERVICE_TIMEOUT_MS` | `120000` | Timeout (parse pode levar minutos) |
...
| `PARSER_MAX_UPLOAD_MB` | `50` | Limite de upload |
| `PARSER_DO_OCR` | `false` | OCR de PDFs escaneados |
