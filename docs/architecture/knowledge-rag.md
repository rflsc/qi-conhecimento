...
| `link` / `html` | `HtmlParser` | Readability + cheerio → Markdown + `blocks[]` (ver [web-import.md](./web-import.md)) |
...
Configuração no **admin → Configurações** (persistida em `llm_configs` no MongoDB, chaves criptografadas).

| Provedor | Onde configurar | Custo |
| --- | --- | --- |
| **Ollama** | Provedor `ollama`, modelo `nomic-embed-text`; `OLLAMA_BASE_URL` no `.env` | Grátis (local) |
| **OpenAI** | Provedor `openai`, chave OpenAI no painel, modelo `text-embedding-3-small` | Pago |

Default em instalação nova: **Ollama** (sem chave).

Reindexar documento existente: `POST /knowledge/documents/{id}/reindex-embeddings`

Concorrência do worker BullMQ (`EMBEDDING_CONCURRENCY` no `.env`): default **2** (Ollama) ou **5** (OpenAI); máximo 20.

## LLM (`LlmService`)

Respostas enriquecidas em `RagService.generateAnswer()` e `POST /messaging/query`.

Configuração no **admin → Configurações**:

| Provedor | Campos | Modelo default |
| --- | --- | --- |
| **Anthropic** | Provedor `anthropic` + chave Anthropic | `claude-haiku-4-5` |
| **OpenAI** | Provedor `openai` + chave OpenAI | `gpt-4o-mini` |

API: `GET/PATCH /llm-config` (somente `admin`).

Sem provedor LLM configurado: resposta em template com citação do chunk principal.
...
| OCR (imagem) | Docling se parser ativo; senão exige chave OpenAI em **Configurações** |
...
| POST | `/knowledge/web-imports` | Cria job de importação web em lote |
| GET/PATCH | `/knowledge/web-imports/settings` | Configurações globais (PATCH: admin) |
| GET | `/knowledge/web-imports/{id}` | Detalhe do job + páginas |
| GET | `/knowledge/web-imports/{id}/stream` | SSE — progresso do job |
...
```env
# Criptografia das chaves salvas no painel (obrigatório em produção)
...
LLM, embeddings e chaves de API: **admin → Configurações** (`/settings`), não no `.env`.
